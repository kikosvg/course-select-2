<template>
  <section class="percent">
    <a-space class="statistic-space" align="center" size="medium" fill>
      ...
      <a-statistic class="statistic-item" title="storage size" :value="totalData.size" :value-style="statisticValueStyle">
        <template #suffix>&nbsp;{{ totalData.unit }}</template>
      </a-statistic>

      <a-statistic class="statistic-item" title="number" :value="totalData.number" :value-style="statisticValueStyle" />
    </a-space>
...
const { chartOption } = useChart(() => {
  return {
    ...
    tooltip: {
      show: true,
      formatter(params) {
        return `total:${params.value}<br>${params.data.size}`
      },
    },
    series: [
      {
