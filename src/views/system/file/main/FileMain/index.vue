  ...
  <div class="file-main">
    <a-row justify="space-between" class="file-main__search">
      ...
      <a-space wrap>
        ...
        <a-input-group>
          <a-input
            v-model="queryForm.name" placeholder="search file name" allow-clear style="width: 200px"
            @change="search"
          />
      ...
      <a-space wrap>
        ...
        <a-button type="primary" @click="isBatchMode = !isBatchMode">
          ...
          <template #default>{{ isBatchMode ? 'cancel batch' : 'batch operation' }}</template>
        </a-button>
        <a-button-group>
...
const onDownload = async (fileInfo: FileItem) => {
  ...
  res ? Message.success('download successfully') : Message.error('download failed')
  search()
}
...
const handleRightMenuClick = async (mode: string, fileInfo: FileItem) => {
  if (mode === 'delete') {
    Modal.warning({
      title: 'warning',
      content: `are you sure to delete the file [${fileInfo.name}]?`,
      hideCancel: false,
...
const handleMulDelete = () => {
  Modal.warning({
    title: 'warning',
    content: `are you sure to delete the ${selectedFileIds.value.length} files?`,
    hideCancel: false,
    onOk: async () => {
