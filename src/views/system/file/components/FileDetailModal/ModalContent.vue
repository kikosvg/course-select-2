    ...
    <a-descriptions :column="1" layout="inline-vertical">
      <a-descriptions-item label="name">
        <a-typography-paragraph copyable :copy-text="data.url">
          <template #copy-tooltip>copy link</template>
          {{ getFileName(data) }}

        </a-typography-paragraph>
      </a-descriptions-item>
      <a-descriptions-item label="size">{{ formatFileSize(data.size) }}</a-descriptions-item>
      <a-descriptions-item label="upload time">{{ data.createTime }}</a-descriptions-item>
      <a-descriptions-item label="update time">{{ data.updateTime }}</a-descriptions-item>
      <a-descriptions-item label="storage name">{{ data.storageName }}</a-descriptions-item>
    </a-descriptions>
