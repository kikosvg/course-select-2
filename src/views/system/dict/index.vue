          ...
          <a-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" :xxl="24" flex="1" class="h-full overflow-hidden">
            ...
            >
              <template #toolbar-left>
                <a-input-search v-model="queryForm.description" placeholder="search label/description" allow-clear @search="search" />
              ...
              <template #action="{ record }">
                <a-space>
                  <a-link v-permission="['system:dict:item:update']" title="modify" @click="onUpdate(record)">modify</a-link>
                  <a-link
...
const columns: TableInstanceColumns[] = [
  ...
  { title: 'label', dataIndex: 'label', slotName: 'label', minWidth: 100, align: 'center' },
  { title: 'value', dataIndex: 'value', minWidth: 100, align: 'center', ellipsis: true, tooltip: true },
...
const onDelete = (record: DictItemResp) => {
  return handleDelete(() => deleteDictItem(record.id), {
    content: `are you sure to delete the dictionary「${record.label}」?`,
    showModal: true,
  })
