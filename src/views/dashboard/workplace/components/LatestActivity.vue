<template>
  <a-card class="general-card" title="Latest Activity" style="margin-bottom: 14px">
    <template #extra>
      <a-dropdown>
        <a-link>more</a-link>
    ...
    <div v-else>
      <a-empty v-if="dataList.length === 0">No activity</a-empty>
      <a-comment
        ...
        <template #content>
          <div class="content">
            <p v-if="item.type === 'PUSH'">
              Pushed to <a-link :href="item.repo.url" target="_blank" rel="noopener">{{ item.repo.alias }}</a-link>
              {{ `the ${item.payload.branch} branch ${item.payload.commits.length} commits` }}
              <a-comment
