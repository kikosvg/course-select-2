<template>
  ...
  >
    <a-form-item field="username" hide-label>
      <a-input v-model="form.username" placeholder="please enter username" allow-clear />
    </a-form-item>
    <a-form-item field="password" hide-label>
      <a-input-password v-model="form.password" placeholder="please enter password" />
    ...
    <a-form-item v-if="isCaptchaEnabled" field="captcha" hide-label>
      ...
      <div class="captcha-container" @click="getCaptcha">
        ...
        <img :src="captchaImgBase64" alt="captcha" class="captcha" />
        <div v-if="form.expired" class="overlay">
          <p>expired, please refresh</p>
        </div>
      </div>
    </a-form-item>
    <a-form-item>
      <a-row justify="space-between" align="center" class="w-full">
        <a-checkbox v-model="loginConfig.rememberMe">remember me</a-checkbox>
        <a-link>forget password</a-link>
      </a-row>
    </a-form-item>

    <a-form-item>
      <a-space direction="vertical" fill class="w-full">
        <a-button class="btn" type="primary" :loading="loading" html-type="submit" size="large" long>login</a-button>
      </a-space>
    </a-form-item>
  </a-form>
...
const rules: FormInstance['rules'] = {
  ...
  captcha: [{ required: isCaptchaEnabled.value, message: 'please enter captcha' }],
}
